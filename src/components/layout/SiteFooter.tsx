...
export function SiteFooter() {
  ...
  const tr = (uz: string, ru: string, en: string, zh: string) => {
    ...
    <footer className="premium-footer text-white">
      <div className="mx-auto max-w-[1480px] px-4 pt-14 sm:px-6">
        <div className="grid gap-10 border-b border-white/10 pb-12 lg:grid-cols-[1.3fr_.85fr_.85fr_1.1fr]">
          <div>
            <div className="flex items-center gap-4">
              <img
                src={EMBLEM}
                alt={tr("Ўзбекистон Республикаси герби", "Герб Республики Узбекистан", "Emblem of the Republic of Uzbekistan", "乌兹别克斯坦共和国国徽")}
                className="size-16 shrink-0 object-contain drop-shadow-[0_8px_14px_rgba(0,0,0,.25)]"
              />
              <div>
            ...
            <p className="mt-5 max-w-md text-sm leading-7 text-white/58">
              {tr(
                "Сурхондарё вилоятида инвестиция муҳитини яхшилаш, саноатни ривожлантириш ва экспорт салоҳиятини ошириш.",
                "Улучшение инвестиционного климата, развитие промышленности и повышение экспортного потенциала Сурхандарьинской области.",
                "Improving the investment climate, developing industry and increasing the export potential of Surkhandarya region.",
                "改善苏尔汉河州投资环境、发展工业并提升出口潜力。",
              )}
            </p>
            ...
            <div className="mt-6 flex gap-2">
              {socials.map(([key, Icon, title]) => (
                <a
                  ...
                  rel="noreferrer"
                  title={title}
                  className="grid size-10 place-items-center rounded-xl border border-white/10 bg-white/5 text-white/75 transition hover:-translate-y-1 hover:border-amber-300/30 hover:bg-amber-300/10 hover:text-amber-300"
                >
          ...
          <div>
            <p className="premium-footer-title">{tr("Бўлимлар", "Разделы", "Sections", "栏目")}</p>
          ...
          <div>
            <p className="premium-footer-title">{tr("Фойдали ҳаволалар", "Полезные ссылки", "Useful links", "实用链接")}</p>
          ...
          <div>
            <p className="premium-footer-title">{tr("Алоқа", "Контакты", "Contacts", "联系方式")}</p>
            <div className="mt-5 grid gap-4 text-sm text-white/58">
              ...
              <a href="tel:+998762241415" className="flex gap-3 hover:text-white">
                <Phone className="size-4 text-amber-300" />
                {CONTACTS.phones[0]}
              </a>
              <a href={`mailto:${CONTACTS.email}`} className="flex gap-3 hover:text-white">
            ...
            <Link to="/contacts" className="mt-6 inline-flex items-center gap-2 text-sm font-extrabold text-amber-300">
              {tr("Боғланиш", "Связаться", "Contact us", "联系我们")}
        ...
        <div className="flex flex-col gap-3 py-5 text-xs text-white/38 sm:flex-row sm:items-center sm:justify-between">
          <p>© {year} {t("org.name")}. {tr("Барча ҳуқуқлар ҳимояланган.", "Все права защищены.", "All rights reserved.", "版权所有。")}</p>
          <p>{tr("Расмий давлат инвестиция портали", "Официальный государственный инвестиционный портал", "Official government investment portal", "官方政府投资门户")}</p>
        </div>
      </div>
    </footer>
  );
