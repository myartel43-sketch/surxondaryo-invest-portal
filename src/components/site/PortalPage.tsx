...
import { listStaff, listDocuments, listMedia, listMapItems, localized, type StaffItem, type DocumentItem, type MediaItem, type MapItem } from "@/lib/extra-content-api";
...
export function PortalPage({ page, title }: { page:string; title:string }) {
  ...
  const [remoteProjects,setRemoteProjects]=useState<ProjectItem[]>([]);
  const [remoteStaff,setRemoteStaff]=useState<StaffItem[]>([]);
  const [remoteDocs,setRemoteDocs]=useState<DocumentItem[]>([]);
  const [remoteMedia,setRemoteMedia]=useState<MediaItem[]>([]);
  const [remoteMap,setRemoteMap]=useState<MapItem[]>([]);
  useEffect(()=>{
    if(page==="news") listNews(true).then(setRemoteNews).catch(()=>setRemoteNews([]));
    if(page==="projects") listProjects(true).then(setRemoteProjects).catch(()=>setRemoteProjects([]));
    if(page==="staff"||page==="leadership") listStaff(true).then(setRemoteStaff).catch(()=>setRemoteStaff([]));
    if(page==="documents") listDocuments(true).then(setRemoteDocs).catch(()=>setRemoteDocs([]));
    if(page==="media") listMedia(true).then(setRemoteMedia).catch(()=>setRemoteMedia([]));
    if(page==="map") listMapItems(true).then(setRemoteMap).catch(()=>setRemoteMap([]));
  },[page]);
  const publicProjects = remoteProjects.length ? remoteProjects.map(p=>({ title:pickLanguage(p,"title",lang), description:pickLanguage(p,"description",lang), sector:p.sector, amount:p.amount, jobs:p.jobs, status:p.status, district:p.district, image_url:p.image_url })) : projects;
  const filteredProjects=useMemo(()=>publicProjects.filter(p=>`${p.title} ${p.sector} ${p.district}`.toLowerCase().includes(query.toLowerCase())),[query,publicProjects]);
  const publicStaff = remoteStaff.length ? remoteStaff.map(x=>({name:localized(x,"name",lang),role:localized(x,"role",lang),phone:x.phone,email:x.email,image_url:x.image_url})) : staff;
  const publicDocs = remoteDocs.length ? remoteDocs.map(x=>({title:localized(x,"title",lang),type:x.file_type,date:x.document_date,href:x.file_url})) : documents;
  const filteredStaff=useMemo(()=>publicStaff.filter(p=>`${p.name} ${p.role}`.toLowerCase().includes(query.toLowerCase())),[query,publicStaff]);
  const filteredDocs=useMemo(()=>publicDocs.filter(p=>p.title.toLowerCase().includes(query.toLowerCase())),[query,publicDocs]);
  ...
  if (page==="projects") content=<><SearchBox value={query} onChange={setQuery} placeholder="Лойиҳа, соҳа ёки туман бўйича қидириш"/><div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{filteredProjects.map((p:any)=><article key={p.title} className="overflow-hidden rounded-2xl border border-border bg-card">{p.image_url&&<img src={p.image_url} alt={p.title} className="h-44 w-full object-cover"/>}<div className="p-6"><span className="text-xs font-semibold uppercase tracking-wide text-green">{p.sector}</span><h3 className="mt-3 text-lg font-bold">{p.title}</h3>{p.description&&<p className="mt-2 line-clamp-3 text-sm leading-6 text-muted-foreground">{p.description}</p>}<div className="mt-5 grid grid-cols-2 gap-3 text-sm"><div><p className="text-muted-foreground">Қиймати</p><b>{p.amount}</b></div><div><p className="text-muted-foreground">Иш ўрни</p><b>{p.jobs}</b></div><div><p className="text-muted-foreground">Ҳудуд</p><b>{p.district}</b></div><div><p className="text-muted-foreground">Ҳолати</p><b>{p.status}</b></div></div></div></article>)}</div></>;
  ...
  else if (page==="map") content=<><div className="overflow-hidden rounded-3xl border border-border bg-card"><iframe title="Yandex map" src="https://yandex.com/map-widget/v1/?ll=67.2783%2C37.2242&z=11" className="h-[520px] w-full" loading="lazy"/></div>{remoteMap.length>0&&<div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{remoteMap.map(x=><article key={x.id} className="rounded-2xl border border-border bg-card p-5"><MapPin className="size-6 text-green"/><h3 className="mt-3 font-bold">{localized(x,"title",lang)}</h3><p className="mt-2 text-sm text-muted-foreground">{localized(x,"description",lang)}</p><p className="mt-3 text-xs font-semibold text-primary">{x.category} · {x.address}</p><a target="_blank" rel="noreferrer" className="mt-4 inline-flex text-sm font-bold text-blue-700" href={`https://yandex.com/maps/?pt=${x.longitude},${x.latitude}&z=15&l=map`}>Харитада очиш →</a></article>)}</div>}</>;
  ...
  else if (page==="news") { const publicNews = remoteNews.length ? remoteNews.map(n=>({title:pickLanguage(n,"title",lang),text:pickLanguage(n,"description",lang),date:n.published_at,image_url:n.image_url})) : news; content=<div className="grid gap-5 md:grid-cols-3">{publicNews.map((n:any)=><article key={`${n.title}-${n.date}`} className="overflow-hidden rounded-2xl border border-border bg-card">{n.image_url?<img src={n.image_url} alt={n.title} className="h-48 w-full object-cover"/>:<div className="flex h-40 items-center justify-center gradient-brand"><Newspaper className="size-12 text-white/80"/></div>}<div className="p-6"><p className="flex items-center gap-2 text-xs text-muted-foreground"><CalendarDays className="size-4"/>{n.date}</p><h3 className="mt-3 text-lg font-bold">{n.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{n.text}</p></div></article>)}</div>; }
  else if (page==="media") content=<div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{remoteMedia.length?remoteMedia.map(x=><article key={x.id} className="overflow-hidden rounded-2xl border border-border bg-card">{x.thumbnail_url||x.media_url?<img src={x.thumbnail_url||x.media_url} alt={localized(x,"title",lang)} className="aspect-video w-full object-cover"/>:<div className="flex aspect-video items-center justify-center gradient-brand"><ImageIcon className="size-12 text-white/75"/></div>}<div className="p-4"><h3 className="font-semibold">{localized(x,"title",lang)}</h3><p className="mt-1 text-sm text-muted-foreground">{localized(x,"description",lang)}</p><a href={x.media_url} target="_blank" rel="noreferrer" className="mt-3 inline-flex text-sm font-bold text-blue-700">Кўриш →</a></div></article>):[1,2,3].map(i=><article key={i} className="overflow-hidden rounded-2xl border border-border bg-card"><div className="flex aspect-video items-center justify-center gradient-brand"><ImageIcon className="size-12 text-white/75"/></div><div className="p-4"><h3 className="font-semibold">Тадбирлар ва учрашувлар — {i}</h3><p className="mt-1 text-sm text-muted-foreground">Фото ва видео материаллар</p></div></article>)}</div>;
